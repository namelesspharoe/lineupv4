...
export function MobileMenu({ isOpen, onClose, onLogout }: MobileMenuProps) {
  return (
    <>
      {/* Mobile Menu Overlay */}
      {isOpen && (
        <div className="fixed inset-0 z-40 bg-black/50 dark:bg-black/60 lg:hidden" aria-hidden />
      )}

      {/* Mobile Navigation Menu */}
      <div
        id="mobile-menu"
        className={`fixed bottom-0 right-0 top-14 z-50 flex w-[min(100vw-2.5rem,20rem)] flex-col border-l border-gray-200 bg-white shadow-xl transition-transform duration-200 ease-out dark:border-gray-800 dark:bg-gray-950 sm:top-16 sm:w-80 lg:hidden ${
